...
import { Button, Drawer, DrawerCloseButton, DrawerContent, Text,Flex, DrawerHeader, DrawerOverlay, Icon, Image, Divider} from '@chakra-ui/react';
...
const HomeDrawe:React.FC<DrawerProps> = () => {
    ...
    return (
        ...
        <Flex
           cursor="pointer"
           bg="white"
           _hover={{ bg: "gray.100" }} onClick={onOpen} >
...
<CreateTeamModel open={open} handleClose={()=>setOpen(false)} />
     ...
     {user?.uid &&    <Text ml={5}   fontWeight={600}>Team Leader</Text>}
           {user?.uid && userTeam.map((team)=>(
            ...
            {team.creatorId===user?.uid && 
             <Flex key={"team"} margin="5px 5px" border="1px solid white"  >
             ...
             <Flex direction="row" cursor="pointer" p={2} borderRadius="full" >
              <>
                  ...
                  {team?.imageUrl ? (
                       <Image borderRadius="full" boxSize="18px" src={team.imageUrl} mr={2} key={"teamImage"} />
    ...
    <Divider/>
        {user?.uid && <Text ml={5}   fontWeight={600}>Teams</Text>}
        {user?.uid && userTeam.map((team)=>(
               <>
              
               <Flex key={team.Name} margin="5px 5px" border="1px solid white"  >
